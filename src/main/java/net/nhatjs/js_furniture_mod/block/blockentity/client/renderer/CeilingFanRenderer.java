...
public class CeilingFanRenderer implements BlockEntityRenderer<CeilingFanBlockEntity> {
    ...
    public CeilingFanRenderer(BlockEntityRendererProvider.Context ctx) {
        blades_black = Minecraft.getInstance().getModelManager().getModel(ModelResourceLocation.inventory(ResourceLocation.fromNamespaceAndPath(MOD_ID, "block/ceiling_fan_blades")));
        blades_white = Minecraft.getInstance().getModelManager().getModel(ModelResourceLocation.inventory(ResourceLocation.fromNamespaceAndPath(MOD_ID, "block/ceiling_fan_blades_b")));
    }
    ...
    public void render(CeilingFanBlockEntity be, float tickDelta,
        ...
        if (blades_black == null || blades_black == Minecraft.getInstance().getModelManager().getMissingModel()
        || blades_white == null || blades_white == Minecraft.getInstance().getModelManager().getMissingModel()) {
            return;
        }
